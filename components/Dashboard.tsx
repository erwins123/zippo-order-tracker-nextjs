...
export default function Dashboard({ orders, onNavigate }: Props) {
  ...
  const cards = [
    { n: orders.length, l: 'Total order rows', cls: 'blue', tab: 'allorders' },
    ...
    { n: resolved.length, l: 'Resolved issues', cls: 'green', tab: 'issues' },
              ...
              {stores.length === 0 ? (
                ...
                return (
                  <tr key={s}>
                    ...
                    <td>{del}</td>
                    <td>{tr}</td>
                    <td style={{ color: op ? '#dc2626' : 'var(--muted)', fontWeight: op ? 700 : 400 }}>{op}</td>
                    <td style={{ color: res ? '#16a34a' : 'var(--muted)' }}>{res}</td>
                  </tr>
                )
              })}
            </tbody>
